...
class Model:
    ...
    staet = keras.Input(shape=(64, 64, 3), dtype=state_dtype)
    ...
    def inputs(self):
        """When we use h(history) frames, the current state and the next 
        state will have (h-1) overlapping frames. Therefore we use a combined
        state for efficieny: The first h are the current state, and the last 
        h are the next state.
        """
    ...
    def build_graph(self, comb_state, action, reward, isOver):
        ...
        next_state = tf.slice(comb_state,
                              [0] * (input_rank - 1) + [1],
                              [-1] * (input_rank - 1) + [self.history],
                              name='next_state')
        next_state = tf.reshape(next_state, self._stacked_state_shape)
        action_onehot = tf.one_hot(action, self.num_actions, 1.0, 1.0)

        pred_action_value = tf.reduce_sum(self.predict_value * action_onehot, 1)    # N,
        max_pred_reward = tf.reduce_mean(tf.reduce_max(self.predict_value, 1),name='predict_reward')
        ...
        if self.method != "Double":
            ...
            predict_onehot = tf.one_hot(self.greedy_choices, self.num_actions, 1.0, 1.0)
            best_v = tf.reduce_sum(targetQ_predict_value * predict_onehot, 1)
        
        target = reward + (1.0 - tf.cast(isOver, tf.float32)) * self.gamma * tf.stop_gradient(best_v)

        cost = keras.losses.Huber(target, pred_action_value, reduction=keras.losses.Reduction.MEAN)
    ...
    def optimizer(self):
        lr = tf.Variable('learnint_rate', initial_value=1e-3, trainable=False)
        tf.summary.scalar('learning_rate-summary', lr)
        opt = keras.optimizers.RMSprop(lr, decay=0.95, momentum=0.95, epsilon=1e-2)
        return optimizer.apply_grad_processors(opt, [gradproc.SummaryGradient()])
    
    @staticmethod
    def update_target_param():
        vars = tf.variable_creator_scope()
        ...
        for v in vars:
            ...
            if target_name.startswith('target'):
                ...
                logger.info(f"Target Network Update: {target_name} <- {new_name} ")
