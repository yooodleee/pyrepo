from patterns.modeling.repository.repository import SqlAlchemyRepository
from patterns.modeling.model.model import Batch, OrderLine
...
def insert_order_line(session):
    ...
    [[orderline_id]] = session.execute(
        ...
        dict(orderid="orderid", sku="GENERIC-SOFA")
    )
    return orderline_id
...
def test_repository_can_retrieve_a_batch_with_allocations(session):
    ...
    assert retrieved == expected    # Batch.__eq__()
...
def get_allocations(session, batchid):
    rows = list(
        session.execute(
            ...
            " WHERE batches.reference = :batched",
...
def test_updating_a_batch(session):
    order1 = OrderLine("order1", "WEATHERED-BENCH", 10)
    order2 = OrderLine("order2", "WEATHERED-BENCH", 20)
